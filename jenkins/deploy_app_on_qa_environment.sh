...
AWS_REGION=$AWS_REGION helm repo add stable-petclinic-rcp s3://petclinic-helm-charts-rcp/stable/myapp/ || echo "repository name already exists"
AWS_REGION=$AWS_REGION helm repo update
helm package k8s/petclinic_chart
AWS_REGION=$AWS_REGION helm s3 push --force petclinic_chart-${BUILD_NUMBER}.tgz stable-petclinic-rcp
...
kubectl delete secret regcred || echo "there is no regcred secret in petclinic-qa namespace"
...
AWS_REGION=$AWS_REGION helm upgrade --install \
    petclinic-app-release stable-petclinic-rcp/petclinic_chart --version ${BUILD_NUMBER} \
